...
public class HashMap<K, V> {
    ...
    public V get(K key) {   // first generates hashvalue for key, then goes using the hashvalue into the list the key is in (index),
                             //and return that value, else return null.
    ...
    public void add(K key, V value) {
          // to add values to hashmap
        // first generates hash value for the key, searches the key to find a list, if not found then create a new list. 
        // search for index of key at list, if found, then key is duplicate; replace value. otherwise, if not index of key, key must be new.
        // add key to list. (which is denoted by its hashvalue).
